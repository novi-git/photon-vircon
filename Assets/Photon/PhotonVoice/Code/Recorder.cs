    ...
    public class Recorder : VoiceComponent
    {
        ...
        private void Setup()
        {
            ...
            if (this.voice == LocalVoiceAudioDummy.Dummy)
            {
                if (this.Logger.IsWarningEnabled)
                {
                    this.Logger.LogWarning("Local input source setup and voice stream creation failed. No recording or transmission will be happening. See previous error log messages for more details.");
                }
                return;
            }
